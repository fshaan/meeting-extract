...
预期输出 JSON：
```json
{"speakers":[{"name":"陕飞","segments":163},{"name":"康方刘淼","segments":84}],"total_segments":308,"title":"04-07 客户会议"}
```
...
- **BOM 头**：脚本内部自动剥离
- **说话人模糊匹配**：用户输入姓名与转写标签做包含关系匹配（如输入"刘淼"匹配"康方刘淼"），匹配不到时提示用户从检测到的说话人列表选择
...
用户: 帮我提取这个会议里陕飞的发言观点，文件是 文本-A8411D47DF72-1.txt
→ 自动识别为"发言提取"类型，目标人员"陕飞" → 生成 → 保存文件
